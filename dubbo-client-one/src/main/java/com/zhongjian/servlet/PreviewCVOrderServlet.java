...
//						String isSelfMention = formData.get("isselfmention") == null?"0":formData.get("isselfmention");
						result = PreviewCVOrderServlet.this.handle(uid,sid,type,extra);
						ResponseHandle.wrappedResponse(asyncContext.getResponse(), result);
					} catch (Exception e) {
						e.printStackTrace();
						try {
	...
	private String handle(Integer uid, Integer sid, String type, Integer extra) throws NDCException {
		if (uid == 0) {
		...
		Map<String, Object> orderDetail = orderService.previewCVOrder(uid, sid, type, extra);
		//请求服务获取预览订单数据--end
		//获取数据
		try {
