...
@WebServlet(value = "/v1/cvorder/cancel", asyncSupported = true)
public class CancelCVOrderServlet extends HttpServlet {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;

	private static Logger log = Logger.getLogger(CancelCVOrderServlet.class);
		...
		inputStream.setReadListener(new ReadListener() {
			...
			public void onAllDataRead() {
				ThreadPoolExecutorSingle.executor.execute(() -> {
					...
					result = CancelCVOrderServlet.this.handle(orderId,uid);
					// 返回数据
						...
						log.error("fail cvorder/cancel : " + e.getMessage());
					}
	...
	private String handle(Integer orderId,Integer uid) {
		...
		if (cvOrderService.cancelOrder(orderId)) {
